...
# Owner display name to actual name mapping (from ESPN usernames to Manager Names)
OWNER_NAME_MAPPING = {
    'Ryan Milhous': 'Ryan Milhous',
    # Jeremy Miller (changed username in 2022)
    'ESPNFAN3533150336': 'Jeremy Miller',  # 2021
    'Jeremy_Miller11': 'Jeremy Miller',    # 2022-2025
    # Jesse Dungan (new owner in 2025)
    'JesseDungan': 'Jesse Dungan',
    # Other current owners
    'TheVervePipe': 'Nick Verville',
    'cbdavis8': 'Casey Davis',
    'kevstach': 'Kevin Stach',
    'Singletarys Winners': 'Tim G',
    'ESPNFAN2999323978': 'Jeffrey Crosswhite',
    'ESPNFAN8340298780': 'Matt McElroy',
    'ESPNFAN8407762097': 'Aaron Van Schyndel',
    'espn00167199': 'Henry Bustamante',
    # Derek Shaw (changed username in 2023)
    'espn77801048': 'Derek Shaw',  # 2022
    'espn74887708': 'Derek Shaw',  # 2023-2025
    # Former league members
    'espn65963844': 'Robert Mandell',  # 2021-2024: New York Nasty Boys - replaced by Jesse Dungan
    'McRobbie30': 'Alex McRobbie',  # 2021: Santa Clara Football Team
}
...
class FantasyDataProcessor:
    ...
    def calculate_best_draft_picks(self):
        """Analyze draft picks to find best and worst value picks for snake draft

        For snake draft:
        - Best value = late picks (high overall_pick number) that scored many points
        - Worst value = early picks (low overall_pick number) that scored few points
        - Value metric = points per draft position, adjusted so early picks "cost" more
        """
        ...
        for draft_pick in self.processed_data['draft']:
            ...
            overall_pick = draft_pick.get('overall_pick', 0)
            round_num = draft_pick.get('round_num', 0)

            # Skip keepers (keeper_status = True)
            if draft_pick.get('keeper_status', False):
                continue

            # Skip if no draft position data
            if overall_pick == 0:
                continue
            ...
            if key in player_season_stats:
                ...
                # Calculate value for snake draft
                # Value = total_points / (151 - overall_pick)
                # This creates a "cost" where pick #1 costs 150, pick #150 costs 1
                # Higher value = better pick
                draft_cost = max(151 - overall_pick, 1)  # Prevent division by zero
                value = round(total_points / draft_cost, 2) if draft_cost > 0 else 0
                ...
                all_picks.append({
                    ...
                    'position': position,
                    'round_num': round_num,
                    'round_pick': draft_pick.get('round_pick', 0),
                    'overall_pick': overall_pick,
                    'total_points': round(total_points, 2),
                    'avg_points_per_game': round(avg_points, 2),
                    'games_played': stats['games'],
                    'value': value  # Value metric for snake draft
                })
        ...
        # For worst picks: early round picks (rounds 1-5) that underperformed
        # These are the busts - high picks that didn't deliver
        worst_picks = [
            pick for pick in all_picks
            if pick['round_num'] <= 5 and pick['total_points'] < 100  # Less than 100 points
        ]
        worst_picks.sort(key=lambda x: x['value'])  # Sort by lowest value first
        ...
        print(f"  - {len(best_picks)} late-round picks (rounds 7+, 50+ points) for best value analysis")
        print(f"  - {len(worst_picks)} early-round busts (rounds 1-5, <100 points) for worst value analysis")
